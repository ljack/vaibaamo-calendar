...
export function PasskeyPrompt() {
    ...
    const [isMinimized, setIsMinimized] = useState(() => {
        if (typeof window !== 'undefined') {
            return localStorage.getItem('vaibaamo_passkey_minimized') === 'true'
        }
        return false
    })

    useEffect(() => {
        localStorage.setItem('vaibaamo_passkey_minimized', String(isMinimized))
    }, [isMinimized])
    ...
    return (
        <div className="bg-white border-b border-gray-200 transaction-all duration-300">
                    ...
                    <div className="flex items-center gap-2">
                        {isMinimized && (
                            <span className="text-sm font-medium text-gray-500">
                                {hasPasskey || hasSupakey ? t('passkey.manageModalTitle') : t('passkey.promoDesktop')}
                            </span>
                        )}
                    </div>
                    <button
                        ...
                        title={isMinimized ? t('passkey.expand') : t('passkey.minimize')}
                    >
                        {isMinimized ? (
